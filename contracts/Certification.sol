...
contract Certification {
    ...
    function generateCertificate(
        ...
        string memory _ipfsHash // Add IPFS hash parameter
    ) public {
        ...
        bytes memory tempEmptyStringNameTest = bytes(certificates[byte_id].creation_date);
        require(tempEmptyStringNameTest.length == 0, "Certificate with given id already exists");
    ...
    function getData(string memory _id) public view returns(string memory, string memory, string memory, string memory, string memory, string memory, bool) {
        ...
        bytes memory tempEmptyStringNameTest = bytes(certificates[byte_id].creation_date);
        require(tempEmptyStringNameTest.length != 0, "Certificate id does not exist");
